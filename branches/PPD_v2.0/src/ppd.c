...
void TAKER_main(uint32_t(*getNext)(queue_t*,queueNode_t**)){
	...
	while(1){
		...
		sem_wait(&mainMutex);
		if(getNext(queue,&prevCandidate) >= 1)
			request = TAKER_takeRequest(queue,prevCandidate);
		else {
			//Mover el HeadPosition al ultimo o primer cilindro dependiendo de la direccion.
			//Calcular el tiempo que tarda en llegar a ese cilindro.
			QMANAGER_toggleDirection(&multiQueue->direction);		//Cambiamos la direccion en la que se mueve el cabezal
			//Volver a usar la funcion getNext que ahora podra encontrar el sector requerido.
		}
		sem_post(&mainMutex);
